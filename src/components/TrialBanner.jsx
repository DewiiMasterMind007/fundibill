...
export default function TrialBanner({ daysRemaining, trialExpired, subscriptionExpired, subscriptionEndDate }) {
  ...
  // UI states
  const [showPlans, setShowPlans]       = useState(false) // plan selector modal
  ...
  const expiredDateLabel = subscriptionEndDate
    ? new Date(subscriptionEndDate).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' })
    : ''

  return (
    <>
      {showPlans && (
        <PlanSelectModal onSelect={handleSelectPlan} onClose={() => setShowPlans(false)} />
      )}
      ...
      >
          ...
          {confirmed ? (
            ...
            <>
              ...
              <span style={{ color: '#cbd5e1' }}>
                Your subscription expired{expiredDateLabel ? ` on ${expiredDateLabel}` : ''}.
              </span>
        ...
        {!confirmed && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
            ...
            >
              {subscriptionExpired ? 'Renew Now' : 'Choose a Plan'}
            </button>
